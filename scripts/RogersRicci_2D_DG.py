...
Ls = 2.0*0.007*100/1.4 #TRIALCODE uplift (first factor) can be used to decrease the gradients
...
#TRIALCODE exp replaced with 1+
F = Dt(w)*v1*dx + Dt(T)*v2*dx \
  - v1*div(w*driftvel)*dx - v2*div(T*driftvel)*dx\
  - (1.0/24.0)*(1-exp(3-phi_s/sqrt(T*T+T_eps*T_eps)))*(v1)*dx \
  + ((1.0/36.0)*T*(1.71*exp(3-phi_s/sqrt(T*T+T_eps*T_eps))-0.71)-0.03*ST)*(v2)*dx \
